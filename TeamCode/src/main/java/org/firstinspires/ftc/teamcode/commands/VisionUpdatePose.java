...
public class VisionUpdatePose extends CommandBase {
	...
	public VisionUpdatePose(VisionAprilTags aprilTags, MecanumDriveSubsystem p_drive, OpMode p_opMode) {
		m_opMode = p_opMode;
	...
	public VisionUpdatePose(VisionAprilTags aprilTags, MecanumDriveSubsystem p_drive, OpMode p_opMode,double runTime) {
		this(aprilTags, p_drive, p_opMode);
		m_Milliseconds = runTime;
	}

	public VisionUpdatePose(VisionAprilTags aprilTags, MecanumDriveSubsystem p_drive, OpMode p_opMode, boolean shouldBeClose) {
		this(aprilTags, p_drive, p_opMode);
...
//        }
	...
	public boolean isFinished() {
		if (m_poseArray.size() != 0) {
			...
			com.acmerobotics.roadrunner.geometry.Pose2d pose =
				new com.acmerobotics.roadrunner.geometry.Pose2d(m_poseArray.get(m_poseArray.size() - 1).getX(),
					m_poseArray.get(m_poseArray.size() - 1).getY(),
					Math.toRadians(m_poseArray.get(m_poseArray.size() - 1).getHeading()));
...
//        }
	...
	public void telemetry() {
        if (m_poseArray.size() > 0)
            m_opMode.telemetry.addData("UpdVuf: ", "size: %d, ms = %.1f, pose: (%.1f, %.1f, %.1f)",
